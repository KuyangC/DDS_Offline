  ...
  Widget _buildLED(int ledIndex, Color activeColor) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Calculate optimal LED size with minimum and maximum constraints
        double maxLEDSize = 24.0;  // Increased from 12.0
        double minLEDSize = 14.0;   // Increased from 8.0
        double spacingBudget = totalAvailableWidth / 6.0;

        // Use 85% of available space for LED, but clamp between min and max
        double responsiveLEDSize = (spacingBudget * 0.85).clamp(minLEDSize, maxLEDSize);

        // Calculate font size based on LED size
        double fontSize = (responsiveLEDSize * 0.5).clamp(8.0, 12.0);
