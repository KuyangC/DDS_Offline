...
class _OfflineMonitoringPageState extends State<OfflineMonitoringPage> with WidgetsBindingObserver {
  ...
  void _onActivityLogsChanged() {
    // 🔥 SAFETY CHECK: Check BOTH mounted AND _disposed
    if (!mounted || _disposed) {
      AppLogger.debug('Skipping _onActivityLogsChanged - widget disposed or not mounted', tag: 'OFFLINE_MONITORING');
      return;
    }

    _initializeDates(); // Reinitialize dates when activity logs change
  }
