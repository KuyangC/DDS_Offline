...
class ConnectionHealthService {
  ...
  static Future<List<DiscoveryResult>> discoverESP32DevicesOptimized({
    ...
    List<int> ports = const [81, 80], // WebSocket port first, then HTTP
    int maxIPRange = 50,
    Duration? timeout = const Duration(milliseconds: 500),
    int maxDevices = 10,
  }) async {
    // Use smart discovery for best performance
    return discoverDevicesSmart(
  ...
  /// ULTRA-FAST: Smart discovery - ARP-first, then parallel fallback
  /// Expected time: 0.5-5 seconds (vs 30-75 seconds before)
  static Future<List<DiscoveryResult>> discoverDevicesSmart({
    List<String>? baseNetworks,
    List<int> ports = const [81, 80],
    int maxIPRange = 50,
    Duration timeout = const Duration(milliseconds: 500),
    int maxDevices = 10,
  }) async {
    AppLogger.info('🚀 Starting SMART device discovery (ports=$ports, timeout=${timeout.inMilliseconds}ms)', tag: _tag);
    final stopwatch = Stopwatch()..start();

    // Step 1: Try ARP-first (instant for known devices)
    var results = await _discoverViaArp(
      ports: ports,
      timeout: timeout,
      maxDevices: maxDevices,
    );
    ...
    if (results.isNotEmpty) {
      stopwatch.stop();
      AppLogger.info('✅ Found ${results.length} devices via ARP in ${stopwatch.elapsedMilliseconds}ms', tag: _tag);
      return results;
    }

    // Step 2: Fallback to parallel batch scanning
    AppLogger.info('🔄 ARP found nothing, falling back to parallel scan', tag: _tag);
    
    // Build network list: auto-detected + common defaults
    final detectedNetwork = await _detectCurrentNetwork();
    final networkSet = <String>{};
    if (detectedNetwork != null) {
      networkSet.add(detectedNetwork);
    }
    // Always include common subnets
    networkSet.addAll(['192.168.0', '192.168.1', '10.0.0']);
    final networks = baseNetworks ?? networkSet.toList();
    
    AppLogger.info('📡 Will scan networks: $networks', tag: _tag);
    
    results = await _discoverParallelBatch(
    ...
    stopwatch.stop();
    AppLogger.info('✅ Parallel scan found ${results.length} devices in ${stopwatch.elapsedMilliseconds}ms', tag: _tag);
    return results;
  }

  /// ARP-based discovery - scan only known active devices
  static Future<List<DiscoveryResult>> _discoverViaArp({
    List<int> ports = const [81, 80],
    ...
    try {
      // Get ARP table (contains ONLY active devices on network)
      final arpTable = await ArpService.getArpTable();
      AppLogger.info('📋 ARP table has ${arpTable.length} active devices', tag: _tag);

      if (arpTable.isEmpty) return results;

      // Sort: Espressif devices first (ESP32)
      final sortedArp = List<ArpEntry>.from(arpTable);
      sortedArp.sort((a, b) {
        if (a.manufacturer == 'Espressif') return -1;
        if (b.manufacturer == 'Espressif') return 1;
        return 0;
      });

      // Test in parallel batches of 10
      const batchSize = 10;
      for (int i = 0; i < sortedArp.length && results.length < maxDevices; i += batchSize) {
        final batch = sortedArp.skip(i).take(batchSize).toList();

        final batchFutures = batch.map((entry) async {
          // Test all ports in parallel for this host
          final portResults = await _testCommonPortsFast(entry.ip, ports: ports, timeout: timeout);
          final reachable = portResults.where((r) => r.isReachable).toList();
          
          if (reachable.isNotEmpty) {
            final bestPort = reachable.first.port;
            final bestTime = reachable.first.responseTime;
            return DiscoveryResult(
              host: entry.ip,
              reachablePorts: reachable.map((p) => PortTestResult(
                port: p.port,
        ...
        for (final r in batchResults) {
          if (r != null) {
            results.add(r);
            final mfr = r.manufacturer != null ? ' (${r.manufacturer})' : '';
            AppLogger.info('🎯 Found device at ${r.host}:${r.bestPort}$mfr', tag: _tag);
          }
        }
      }
    } catch (e, stackTrace) {
      AppLogger.error('ARP discovery error', tag: _tag, error: e, stackTrace: stackTrace);
    }

    return results;
  }

  /// Parallel batch scanning - test multiple IPs simultaneously
  static Future<List<DiscoveryResult>> _discoverParallelBatch({
    required List<String> baseNetworks,
    List<int> ports = const [81, 80],
    int maxIPRange = 50,
    Duration timeout = const Duration(milliseconds: 500),
    int maxDevices = 10,
    ...
    for (final network in baseNetworks) {
      AppLogger.info('🔍 Scanning $network.x (batch size: $batchSize)', tag: _tag);

      for (int start = 1; start <= maxIPRange && results.length < maxDevices; start += batchSize) {
        final end = (start + batchSize - 1).clamp(1, maxIPRange);

        // Create parallel futures for this batch
        final batchFutures = <Future<DiscoveryResult?>>[];
        for (int i = start; i <= end; i++) {
          final host = '$network.$i';
          batchFutures.add(_testHostMultiPort(host, ports, timeout, arpMap[host]));
        }
  ...
  /// Test single host and return DiscoveryResult if reachable
  static Future<DiscoveryResult?> _testHostSingle(
    String host,
    int port,
    Duration timeout,
    ArpEntry? arpEntry,
  ) async {
    final result = await _testPortFast(host, port, timeout);
    if (result.isReachable) {
      return DiscoveryResult(
        host: host,
        reachablePorts: [PortTestResult(
          port: port,
          result: ConnectionTestResult(isReachable: true, responseTime: result.responseTime),
        )],
        bestPort: port,
  ...
  /// Test host with multiple ports in parallel
  static Future<DiscoveryResult?> _testHostMultiPort(
    String host,
    List<int> ports,
    Duration timeout,
    ArpEntry? arpEntry,
  ...
  /// Auto-detect current WiFi network subnet
  static Future<String?> _detectCurrentNetwork() async {
    try {
        ...
        for (final addr in interface.addresses) {
          if (!addr.address.startsWith('127.')) {
            ...
            if (parts.length == 4) {
              ...
              AppLogger.info('📡 Detected network: $network', tag: _tag);
              return network;
            }
          }
        }
      ...
      AppLogger.warning('Failed to detect network: $e', tag: _tag);
    }
    return null;
  }
