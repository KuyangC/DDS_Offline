...
class AutoRefreshService {
  ...
  void _checkDisconnectStatus() {
    try {
      ...
      // Step 2: Hot reload setelah 30 detik
      else if (_isDisconnected && timeSinceLastData > _disconnectTimeout) {
        print('');
        print('   ⏰⏰⏰ 30 DETIK BERLALU - MELAKUKAN HOT RELOAD! ⏰⏰⏰');
        print('   🔄 Memulihkan aplikasi...');

        // Hapus timer disconnect check untuk mencegah double reload
      ...
      // 🔥 REMOVED: Logic reconnect ini redundant dengan onDataReceived()
      // Disconnect status akan di-clear oleh onDataReceived() ketika ada data masuk
      // Tidak perlu cek isWSConnected di sini
  ...
  /// Schedule hot reload setelah waktu tertentu
  void _scheduleHotReload(Duration timeSinceLastData) {
    ...
    print('   ⏳ Scheduling HOT RELOAD in ${remainingTime.inSeconds} seconds...');

    _hotReloadTimer?.cancel();
    _hotReloadTimer = Timer(remainingTime, () {
      print('');
      print('   ⏰⏰⏰ 30 DETIK TIMER EXPIRED - TRIGGERING HOT RELOAD! ⏰⏰⏰');
      _performHotReload();
    });
  }

  /// Perform hot reload aplikasi
  void _performHotReload() {
    try {
      ...
      print('============================================================');
      print('   Emitting hot reload event to main.dart...');
      print('============================================================');
      print('');

      // Cancel semua timer untuk mencegah double execution
      _disconnectCheckTimer?.cancel();
      _hotReloadTimer?.cancel();

      // Emit event ke stream untuk trigger Phoenix.rebirth di main.dart
      if (!_hotReloadTriggerController.isClosed) {
        _hotReloadTriggerController.add(null);
        print('   ✅ Hot reload event emitted successfully');
      } else {
        print('   ❌ Hot reload controller is closed - cannot emit event');
      }
